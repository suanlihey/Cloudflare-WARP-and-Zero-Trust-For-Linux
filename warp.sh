...
load_remote_lang() {
    ...
    if [ $? -eq 0 ] && [ -n "$lang_data" ]; then
        L[menu_title]=$(echo "$lang_data" | grep -E "L_${choice}_menu_title\s*=" | cut -d'"' -f2)
        L[hw_info]=$(echo "$lang_data" | grep -E "L_${choice}_hw_info\s*=" | cut -d'"' -f2)
        L[opt_1]=$(echo "$lang_data" | grep -E "L_${choice}_opt_1\s*=" | cut -d'"' -f2)
        L[opt_2]=$(echo "$lang_data" | grep -E "L_${choice}_opt_2\s*=" | cut -d'"' -f2)
        L[opt_0]=$(echo "$lang_data" | grep -E "L_${choice}_opt_0\s*=" | cut -d'"' -f2)
        L[back_hint]=$(echo "$lang_data" | grep -E "L_${choice}_back_hint\s*=" | cut -d'"' -f2)
        L[ctrl0_hint]=$(echo "$lang_data" | grep -E "L_${choice}_ctrl0_hint\s*=" | cut -d'"' -f2)
        L[installing]=$(echo "$lang_data" | grep -E "L_${choice}_installing\s*=" | cut -d'"' -f2)
        L[verifying]=$(echo "$lang_data" | grep -E "L_${choice}_verifying\s*=" | cut -d'"' -f2)
        L[mtu_detect]=$(echo "$lang_data" | grep -E "L_${choice}_mtu_detect\s*=" | cut -d'"' -f2)
        L[done]=$(echo "$lang_data" | grep -E "L_${choice}_done\s*=" | cut -d'"' -f2)
        L[fail]=$(echo "$lang_data" | grep -E "L_${choice}_fail\s*=" | cut -d'"' -f2)
        L[team_ask]=$(echo "$lang_data" | grep -E "L_${choice}_team_ask\s*=" | cut -d'"' -f2)
        L[token_ask]=$(echo "$lang_data" | grep -E "L_${choice}_token_ask\s*=" | cut -d'"' -f2)
        L[name_ask]=$(echo "$lang_data" | grep -E "L_${choice}_name_ask\s*=" | cut -d'"' -f2)
        L[mode_ask]=$(echo "$lang_data" | grep -E "L_${choice}_mode_ask\s*=" | cut -d'"' -f2)
        L[port_ask]=$(echo "$lang_data" | grep -E "L_${choice}_port_ask\s*=" | cut -d'"' -f2)
    fi
...
case "\$1" in
    monitor)
        ...
        echo "\$rx_now" > "$RX_CACHE"
        if ! curl -s --max-time 2 "$DETECT_URL" | grep -q "warp=on"; then
...
while true; do
    clear
    echo "======================================="
    echo "   ${L[menu_title]:-Cloudflare WARP Adaptor}"
    echo "======================================="
    printf "${L[hw_info]:-HW: %s | RAM: %s}\n" "$HW_PROFILE" "$TOTAL_MEM"
    echo "---------------------------------------"
    echo "${L[opt_1]:-1. Deploy}"
    echo "${L[opt_2]:-2. Uninstall}"
    echo "${L[opt_0]:-0. Exit}"
    echo "---------------------------------------"
    echo -e "\033[33m${L[back_hint]:-Input 0 back}\033[0m"
