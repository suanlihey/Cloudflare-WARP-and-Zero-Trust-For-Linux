...
L[opt\_1]=$(echo "$lang\_data" | grep -E "L*${choice}_opt_1\s*=" | cut -d'"' -f2)
L[opt_2]=$(echo "$lang_data" | grep -E "L_${choice}opt_2\s*=" | cut -d'"' -f2)
L[opt_0]=$(echo "$lang_data" | grep -E "L${choice}_opt_0\s*=" | cut -d'"' -f2)
L[back_hint]=$(echo "$lang_data" | grep -E "L_${choice}back_hint\s*=" | cut -d'"' -f2)
L[ctrl0_hint]=$(echo "$lang_data" | grep -E "L${choice}_ctrl0_hint\s*=" | cut -d'"' -f2)
L[installing]=$(echo "$lang_data" | grep -E "L_${choice}installing\s*=" | cut -d'"' -f2)
L[verifying]=$(echo "$lang_data" | grep -E "L${choice}_verifying\s*=" | cut -d'"' -f2)
L[mtu_detect]=$(echo "$lang_data" | grep -E "L_${choice}mtu_detect\s*=" | cut -d'"' -f2)
L[done]=$(echo "$lang_data" | grep -E "L${choice}_done\s*=" | cut -d'"' -f2)
L[fail]=$(echo "$lang_data" | grep -E "L_${choice}fail\s*=" | cut -d'"' -f2)
L[team_ask]=$(echo "$lang_data" | grep -E "L${choice}_team_ask\s*=" | cut -d'"' -f2)
L[token_ask]=$(echo "$lang_data" | grep -E "L_${choice}token_ask\s*=" | cut -d'"' -f2)
L[name_ask]=$(echo "$lang_data" | grep -E "L${choice}_name_ask\s*=" | cut -d'"' -f2)
L[mode_ask]=$(echo "$lang_data" | grep -E "L_${choice}mode_ask\s*=" | cut -d'"' -f2)
L[port_ask]=$(echo "$lang_data" | grep -E "L${choice}_port_ask\s*=" | cut -d'"' -f2)fiFallback if parsing fails or specific language is missing[ -z "${L[menu_title]:-}" ] && set_default_lang}--- 5. Hardware Sensing ---get_mem_limit() {local mem_limit_bytesif [ -f /sys/fs/cgroup/memory/memory.limit_in_bytes ]; thenmem_limit_bytes=$(cat /sys/fs/cgroup/memory/memory.limit_in_bytes)if [ "$mem_limit_bytes" -lt 9223372036854771712 ]; thenecho $((mem_limit_bytes / 1024 / 1024))returnfififree -m | awk '/^Mem:/{print $2}'}TOTAL_MEM=$(get_mem_limit)if [ "$TOTAL_MEM" -le 300 ]; thenHW_PROFILE="LOW"; MEM_LIMIT="100M"; LOG_LEVEL="off"; RESTART_DELAY="15"elif [ "$TOTAL_MEM" -le 1024 ]; thenHW_PROFILE="MED"; MEM_LIMIT="256M"; LOG_LEVEL="error"; RESTART_DELAY="10"elseHW_PROFILE="HIGH"; MEM_LIMIT="512M"; LOG_LEVEL="info"; RESTART_DELAY="5"fi[ "$EUID" -eq 0 ] && mkdir -p "$TMP_SANDBOX" && export TMPDIR="$TMP_SANDBOX"--- 6. Core Functions ---discover_mtu() {echo "${L[mtu_detect]:-Detecting MTU...}"local wait_count=0while ! ip link show | grep -E "CloudflareWARP|warp" >/dev/null 2>&1; dosleep 1; ((wait_count++)); [ $wait_count -gt 15 ] && breakdonelocal best_mtu=0for test_mtu in 1420 1380 1350; doif ping -c 1 -W 1 -M do -s $((test_mtu - 28)) 1.1.1.1 >/dev/null 2>&1; thenbest_mtu=$test_mtu; breakfidone[ "$best_mtu" -eq 0 ] && best_mtu=1380warp-cli set-mtu "$best_mtu" &>/dev/null}install_warp() {echo "${L[installing]:-Installing...}"
